...
let foundMax = findIntersectionsOnLine(lines[0], lines[1]);
console.log('finished @ '+foundMax);
...
function findIntersections(a, b){
    var pts = _intersect(a, b)
    if (pts){
        pts.forEach(pt => {
            console.log('intersect %s %s = %s', logVector(a), logVector(b), logPt(pt));
            let dist = getDistance(p0, pt);
            if (dist>0 && dist < max){
                max = dist
                console.log('Found dist %d @ %s', dist, logPt(pt));
            }
        })
    }
}
